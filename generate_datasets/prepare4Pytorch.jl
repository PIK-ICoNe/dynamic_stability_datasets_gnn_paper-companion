...
const snbs_threshold = .1
const surv_threshold = 15.
const K = 9.

function compute_snbs(input_frequency, threshold)
    N = size(input_frequency, 1)
    snbs = zeros(N)
    num_pert_per_node = size(input_frequency, 2)
    conv = input_frequency .< threshold
    for n in 1:N
...
for r in grid_index_start:grid_index_end
    ...
    snbs = compute_snbs(max_final_frequency, snbs_threshold)
    surv = compute_snbs(max_frequency_dev, surv_threshold)
    file_name_label_snbs = string("4Pytorch/snbs_", id, ".h5")
    h5open(file_name_label_snbs, "w") do file
        write(file, "snbs", snbs) 
    end
    file_name_label_surv = string("4Pytorch/surv_", id, ".h5")
    h5open(file_name_label_surv, "w") do file
        write(file, "surv", surv) 
    end
end
